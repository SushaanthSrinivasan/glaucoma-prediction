...
import UploadAndDisplayImage from "../components/UploadAndDisplayImage/UploadAndDisplayImage1";
...
export default function Home() {
	...
	async function sendFilesToBackend() {
		...
		try {
			...
			img2Data = await toBase64(image2);
		} catch (error) {
			console.error(error);
		}

		bodyFormData.append("image1String", img1Data);
		bodyFormData.append("image2String", img2Data);
		...
		setAns("Loading...");
		axios
			.post("http://localhost:5000/classify", bodyFormData, {
				headers: headers,
			})
			.then((response) => {
			...
			<div>
				...
				<HStack spacing={8}>
					<Flex height="20vh" width="50vw" justifyContent="center">
						<UploadAndDisplayImage sendDataToParent={sendDataToParent} />
					</Flex>
					<Flex height="20vh" width="50vw" justifyContent="center">
						<VStack spacing={8}>
